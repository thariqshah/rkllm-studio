...
unsafe extern "C" fn rkllm_callback_wrapper(result: *mut RKLLMResult, userdata: *mut c_void, state: LLMCallState) {
    if result.is_null() || userdata.is_null() { return; }
...
impl RKLLMEngine {
    ...
    pub async fn run(&self, prompt: &str, tx: mpsc::Sender<String>) -> Result<(), String> {
        ...
        let c_role = CString::new("user").unwrap();
        ...
        let result = tokio::task::spawn_blocking(move || {
            unsafe {
                ...
                let role_ptr = c_role.as_ptr();

                // Context is local to this thread and lives for the duration of rkllm_run
                let ctx = RequestCtx { tx };
                let userdata = &ctx as *const RequestCtx as *mut c_void;
                ...
                let ret = rkllm_run(handle, &mut input, &mut infer_param, userdata);
                if ret != 0 {
